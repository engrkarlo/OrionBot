...
const normalizeBlocks = (blocks) => {
  ...
  for (const block of blocks) {
    ...
    if (!['text', 'separator', 'section', 'image', 'field', 'button', 'footer'].includes(block.type)) {
      throw new Error(`Unsupported component type: ${block.type}`)
    }
  ...
  return blocks.map((block) => {
    ...
    if (block.type === 'field') {
      const [field] = normalizeFields([block])
      return { type: 'field', ...field }
    }
    if (block.type === 'button') {
      const [button] = normalizeButtons([block])
    ...
    if (block.type === 'separator') {
      return {
        ...
        spacing: block.spacing === 'large' ? 'large' : 'small',
      }
    }
...
const buildComponentsV2Embed = (input = {}) => {
  ...
  while (index < blocks.length) {
    ...
    if (block.type === 'image') {
      ...
      while (index < blocks.length && blocks[index].type === 'image') {
        gallery.addItems(new MediaGalleryItemBuilder().setURL(blocks[index].url))
        index += 1
      }
    ...
    if (block.type === 'field') {
      const fieldParts = []
      while (index < blocks.length && blocks[index].type === 'field') {
        fieldParts.push(`**${blocks[index].name}**\n${blocks[index].value}`)
        index += 1
      }
    ...
    if (block.type === 'button') {
      ...
      while (index < blocks.length && blocks[index].type === 'button' && row.components.length < MAX_BUTTONS) {
        row.addComponents(
          new ButtonBuilder().setLabel(blocks[index].label).setURL(blocks[index].url).setStyle(ButtonStyle.Link)
        )
        index += 1
      }
  ...
  if (container.components.length > 10) {
    throw new Error('This layout creates more than Discord allows inside one container. Remove or combine some components.')
  }
