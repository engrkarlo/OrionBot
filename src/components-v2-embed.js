...
const buildComponentsV2Embed = (input = {}) => {
  ...
  while (index < blocks.length) {
    ...
    if (block.type === 'section') {
      if (!block.thumbnail) container.addTextDisplayComponents(new TextDisplayBuilder().setContent(block.content))
      else {
        const section = new SectionBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(block.content))
        section.setThumbnailAccessory(new ThumbnailBuilder({ media: { url: block.thumbnail } }))
    ...
    if (block.type === 'button') {
      ...
      while (index < blocks.length && blocks[index].type === 'button' && row.components.length < MAX_BUTTONS) { row.addComponents(new ButtonBuilder().setLabel(blocks[index].label).setURL(blocks[index].url).setStyle(ButtonStyle.Link)); index += 1 }
...
module.exports = { buildComponentsV2Embed, isHttpUrl, normalizeColor, normalizeFields, normalizeButtons, normalizeBlocks }
