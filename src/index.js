...
// Restore persisted server config from data.json if present
;(() => {
  try {
    const raw = fs.readFileSync('./src/data.json', 'utf8')
    const savedData = JSON.parse(raw)
    if (savedData.serverConfig) {
      const sc = savedData.serverConfig
      if (sc.ip) mcserver.ip = sc.ip
      if (sc.port) mcserver.port = sc.port
      if (sc.type) mcserver.type = sc.type
      if (sc.name) mcserver.name = sc.name
      if (sc.version) mcserver.version = sc.version
      if (sc.icon) mcserver.icon = sc.icon
    }
  } catch (_) {}
})()
