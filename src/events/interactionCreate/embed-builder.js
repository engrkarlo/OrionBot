...
const { getSavedEmbed } = require('../../saved-embeds')
...
const { listSavedEmbeds, addTrigger, removeTrigger, deleteSavedEmbed } = require('../../saved-embeds')
...
const sendSavedToChannel = async (interaction, record, channel) => {
...
const handleSavedManager = async (interaction, parsed) => {
  ...
  if (interaction.isModalSubmit() && action === 'modal-trigger') {
    session.pending = { ...(session.pending || {}), trigger: interaction.fields.getTextInputValue('trigger').trim() }
  ...
  if (interaction.isChannelSelectMenu()) {
    ...
    if (action === 'send-channel') {
      await sendSavedToChannel(interaction, record, channel)
      session.pending = null
      await interaction.update({ flags: MessageFlags.IsComponentsV2, components: buildManagerComponents(session) })
      return
    }
  ...
  if (action === 'edit') {
    const record = getSavedEmbed(session.guildId, session.selected)
    if (!record) throw new Error('That saved message no longer exists.')
    const builderSession = require('../../embed-builder').createSession(interaction.user.id, interaction.channelId, record)
  ...
  if (action === 'trigger') {
    await interaction.showModal(triggerModal(session))
    return
  }
  if (action === 'delete') {
    const record = getSavedEmbed(session.guildId, session.selected)
...
const updateManager = async (interaction, session) => {
  await interaction.update({ flags: MessageFlags.IsComponentsV2, components: buildManagerComponents(session) })
}
  ...
  try {
    ...
    if (interaction.isModalSubmit()) {
      if (action === 'modal-color') {
        ...
        session.color = normalizeOptionalColor(raw)
        await updateEditor(interaction, session)
        return
      }
      if (action === 'modal-save') {
        const name = interaction.fields.getTextInputValue('name').trim()
        saveSession(session, interaction.guildId, name)
        await updateEditor(interaction, session)
        return
      }
      if (action === 'modal') {
    ...
    if (action === 'save') {
      await interaction.showModal(saveModal(session))
      return
    }
    ...
    if (action === 'send') {
      ...
      await interaction.update({ flags: MessageFlags.IsComponentsV2, components: [statusContainer('## Sent!\n\nYour Components V2 message has been posted in this channel. You can keep editing or save it for reuse.', 0x57f287), ...buildReplyComponents(session)] })
      return
    }
    if (action === 'reset') {
