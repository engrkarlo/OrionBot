...
- **Suggestions System**: Messages sent in configured channels are automatically turned into a suggestion embed with a discussion thread attached, upvote/downvote buttons for members, and approve/reject buttons for staff. Colors, button styles/emojis, channels, and who can submit/vote/moderate are all configurable.
- **Reviews System**: Members leave a star-rated review via a button or the `/review` command (pick a rating, then write the review) and it's posted as a fully customizable embed — title, author line, rating/reviewed fields, colors per rating, and footer are all configurable — with Report / Submit A Review / Useful buttons and a discussion thread attached, similar to app review cards like ReviewHub. Use `/review-panel` to post the sticky submission panel and `/review-delete` to remove a review as staff.
